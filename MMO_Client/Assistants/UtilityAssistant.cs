...
namespace MMO_Client.Code.Assistants
{
    public class UtilityAssistant : StartupScript
    {
        ...
        public static Vector3 ScrenToMapPoint(Vector2 mouseScreenPos, CameraComponent camera = null)
        {
            try
            {
                PlayerController ent = Controller.controller.playerController;
        ...
        /// <summary>
        /// Obtain the angule between two sides who are builded from a puppet and an entity (pobably always the player).
        /// </summary>
        /// <param name="plyr">Entity, The position of the free point, probably the player, 
        /// this is the place where you place the player if you want to detect his position related to the puppet.</param>
        /// <param name="ent">Puppet, The position of the center from where you are calculating</param>
        /// <returns>Returns the angle between both sides as a double, the range is from -180 to 180, in degrees</returns>
        public static double AngleOfRotation(Puppet ent, Entity plyr, UtilityAssistant.Axis axisToignore = UtilityAssistant.Axis.Y)
        {
            try
            {
                ...
                float playx = plyr.Transform.WorldMatrix.TranslationVector.X != 0 ? plyr.Transform.WorldMatrix.TranslationVector.X : plyr.Transform.Position.X;
                float playy = plyr.Transform.WorldMatrix.TranslationVector.Y != 0 ? plyr.Transform.WorldMatrix.TranslationVector.Y : plyr.Transform.Position.Y;
                float playz = plyr.Transform.WorldMatrix.TranslationVector.Z != 0 ? plyr.Transform.WorldMatrix.TranslationVector.Z : plyr.Transform.Position.Z;

                float fwdx = ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.X != 0 ? ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.X : ent.Entity.Transform.Position.X;
                float fwdy = ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.Y != 0 ? ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.Y : ent.Entity.Transform.Position.Y;
                float fwdz = ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.Z != 0 ? ent.RealEnt.GetChildren().Where(d => d.Name == "Fwd").First().Transform.WorldMatrix.TranslationVector.Z : ent.Entity.Transform.Position.Z;

                if (axisToignore == UtilityAssistant.Axis.Y)
                {
                    v1 = new Vector2(playx, playz); //new Vector2(7,2);
                    v2 = new Vector2(fwdx, fwdz); //new Vector2(2,9);

                    v3 = new Vector2(ent.RealEnt.Transform.WorldMatrix.TranslationVector.X, ent.Entity.Transform.WorldMatrix.TranslationVector.Z); //new Vector2(2,9);
                }
                else if (axisToignore == UtilityAssistant.Axis.Z)
                {
                    v1 = new Vector2(playx, playy); //new Vector2(7,2);
                    v2 = new Vector2(fwdx, fwdy); //new Vector2(2,9);
                ...
                //Console.Write("\r{0}%   ","----------------- ");
                //Console.Write("\r total:{0} anglePlayerMob:{1} angleFwdMob:{2} ", total, anglePlayerMob, angleFwdMob);
                //Console.Write("\r{0}%   ",angle);
                //Console.Write("\r{0}%   ","----------------- ");
                //Console.ResetColor();
            ...
            {
                Console.WriteLine("Error AngleOfDoom: " + ex.Message);
                return 0;
            }
        }
        ...
        /// <summary>
        /// Obtain a complete Triangle from one angle and two sides
        /// </summary>
        ...
        /// <returns>A Full Solved triangle (3 sides and 3 angles in grades)</returns>
        public static Triangle TriangleSolve(double AngleA, double SideB, double SideC)
        {
            try
            {
                //Law of Cosines (S A S)
                double SideA = LawOfCosineSAS(AngleA, SideB, SideC);

                //LawofSines
                double AngleB = LawOfSine(AngleA, SideA, SideB);

                double AngleC = 180 - AngleB - AngleA;

                return new Triangle(SideA, SideB, SideC, AngleA, AngleB, AngleC);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error Triangle: " + ex.Message);
                return new Triangle();
            }
        }

        public static double AngleOfDoomGuyMedusa(Puppet ent, Entity plyr)
        {
            //TODO:
            //Obtener pos de monstruo en sí (Que será adaptado para ser el modificador a 0,0)
            //Obtener pos de la punta frontal del monstruo en sí (Que debe ser modificado hacia 0,0)
            //Obtener posición del player (Que debe ser modificado hacia 0,0)
            //

            double ax = 0.0, ay = 1.0, az = 0.0;
            double bx = 0.0, by = -1.0, bz = 0.0;

            double cross = Math.Sqrt(ax * ax + ay * ay + az * az);
